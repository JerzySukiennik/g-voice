# Trening VoxG na Kaggle — instrukcja

Ten sam plan co przy [[microg|MicroG]] i [[gedit|Gedit]]: **30 h GPU tygodniowo za darmo** (T4 ×2), sesja ucinana po 12 h, wszystko poniżej zbudowane pod wznawianie.

VoxG to **dwa** osobne treningi (standard w TTS): najpierw model akustyczny (fonemy → mel-spektrogram), potem osobno wokoder (mel → fala dźwiękowa). Oba trenują niezależnie z tego samego datasetu `voxg-data`.
...
W przeciwieństwie do MicroG (publiczny korpus PL) i Gedit (publiczny InstructPix2Pix), dane VoxG to **Twój własny głos**. Zbiera je osobna PWA (`../recorder`, robi ją drugi agent) do Firebase. Ten potok jest napisany i przetestowany na danych syntetycznych (`tests/smoke_test.py`) — realny trening rusza dopiero, gdy nagrasz materiał. **Nie odpalaj treningu na przykładowych/syntetycznych danych — nic sensownego z tego nie wyjdzie.**

---

## Jak wygląda dataset wejściowy

Eksport z Firebase (jeszcze nie istnieje) ma wyprodukować folder w tym układzie, spakowany i wgrany na Kaggle jako Dataset (np. `voxg-recordings`):
...
6. Na stronie wersji: **Output → New Dataset**, nazwij **`voxg-data`**

Skrypt instaluje `espeak-ng` (apt) + `phonemizer`, uruchamia G2P (tekst→fonemy IPA, PL), wyciąga mel-spektrogramy, F0 i energię, i pakuje wszystko do `voxg_*.bin` + `voxg_meta.json` + `voxg_phonemes.json`. **Bez `--allow-g2p-fallback`** — realny trening musi mieć prawdziwe fonemy espeak-ng, nie zastępczy tryb testowy.
...
6. Po zakończeniu (albo ucięciu na 12h): **Output → New Dataset**, nazwij **`voxg-acoustic-ckpt`**
...
Dokładnie jak krok 2, ale wklej [`03-train-vocoder.py`](03-train-vocoder.py), a output nazwij **`voxg-vocoder-ckpt`**. Wokoder potrzebuje **tylko** `voxg-data` (audio + mele) — NIE checkpointu akustycznego. To GAN (HiFi-GAN), więc trenuje się dłużej niż model akustyczny; `STEPS` znów jest sufitem, oceniaj po tym, jak brzmi zsyntezowane audio.

## Wznowienie (jeśli 12 h nie starczyło)

Nowy notebook jak w kroku 2/3, ale **Add Input** dodatkowo checkpoint (`voxg-acoustic-ckpt` albo `voxg-vocoder-ckpt`). Skrypt sam wykryje `ckpt.pt` i podejmie od ostatniego kroku — razem z momentami optymalizatora (u wokodera: OBU optymalizatorów i OBU sieci), więc bez skoku loss. Po każdej sesji nadpisuj checkpoint nowym outputem (**Output → Update Dataset**).
...
- **Zastępczo (domyślnie):** jeśli nie ma duracji, `prepare_dataset.py` rozdziela klatki **równomiernie** po fonemach. To sprawia, że potok działa, ale daje **robotyczne, równo-rytmiczne** audio. `voxg_meta.json` liczy, ile nagrań poszło na placeholderze (`placeholder_durations`), a prep głośno o tym krzyczy w logu.
